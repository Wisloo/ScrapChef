#!/usr/bin/env python
"""
Test script for SBERT-based recipe recommender.
Loads precomputed embeddings and tests similarity search.
"""
...
def load_embeddings():
    ...
    embeddings = np.load('vector_store/recipe_embeddings.npy')
...
_RECIPE_METADATA = load_recipe_metadata()
    """
    Recommend recipes based on scrap text and optional weight.

    :param scrap_text: Description of food scraps (e.g., "tomato, onion")
    :param weight: Weight from IoT scale (optional)
    :param top_k: Number of top recipes to return
    :param max_weight: Maximum expected weight for normalization (adjust as needed)
    :return: List of (recipe_id, similarity_score) tuples
    """
    # Load SBERT model
